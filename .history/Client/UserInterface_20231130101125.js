export default class UI {
    ...
    addNewTag() {
        this.querySelector('.add-tag').addEventListener('click', () => {
            ...
            const tagText = `Tag ${this.querySelectorAll('.tag').length + 1}`;
            newTag.innerHTML = `<span class="tag-text">${tagText}</span> <button class="delete-tag" style="background-color: ${bgColor};">x</button>`;
            const addTagButton = this.querySelector('.add-tag');
            addTagButton.parentNode.insertBefore(newTag, addTagButton);

            // Add event listener to delete button
            newTag.querySelector('.delete-tag').addEventListener('click', (e) => {
                e.stopPropagation();
                newTag.remove();
                this.saveTags();
            });
            ...
            newTag.querySelector('.tag-text').addEventListener('click', (e) => {
                e.stopPropagation();
    ...
    loadTags() {
        ...
        tags.forEach(tag => {
            const newTag = document.createElement('li');
            newTag.className = 'menu-item tag';
            newTag.style.backgroundColor = tag.bgColor;
            newTag.style.color = 'black';
            newTag.innerHTML = `<span class="tag-text">${tag.text}</span> <button class="delete-tag" style="background-color: ${tag.bgColor};">x</button>`;
            const addTagButton = this.querySelector('.add-tag');
            addTagButton.parentNode.insertBefore(newTag, addTagButton);
    
            // ...existing code...
    
            // Save tags to localStorage after a tag is deleted
            newTag.querySelector('.delete-tag').addEventListener('click', (e) => {
                e.stopPropagation();
                newTag.remove();
                this.saveTags();
            ...
            // Save tags to localStorage after the tag text is edited
            newTag.querySelector('.tag-text').addEventListener('click', (e) => {
                // ...existing code...
