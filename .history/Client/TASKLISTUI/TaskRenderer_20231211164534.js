...
export default class TaskRenderer {
    ...
    renderTasks() {
     ...
     // Add this method to handle arrow button click event using event delegationhandleTaskListClick(event) {
    ...
    async handleArrowButtonClick(task) {
        ...
        // Toggle the visibility of the right menu card
        if (this.rightMenuCard.style.display === 'none' || this.rightMenuCard.style.display === '') {
            this.rightMenuCard.style.display = 'block';
        } else {
            this.rightMenuCard.style.display = 'none';
            return; // Return early if the right menu card is closed
        }
