export default class UI {
    ...
    addNewTag() {
        this.querySelector('.add-tag').addEventListener('click', () => {
            ...
            newTagElement.className = 'menu-item new-tag';
            newTagElement.innerHTML = `Tag ${this.querySelectorAll('.new-tag').length + 1}`;
            newTagElement.style.setProperty('--dynamic-color-card', this.generateRandomColor());
            const addTagButton = this.querySelector('.add-tag');
            addTagButton.parentNode.insertBefore(newTagElement, addTagButton);

            // Add event listener to delete button
            newTagElement.querySelector('.delete-tag').addEventListener('click', () => {
                newTagElement.remove();
                // Update localStorage
                tags = tags.filter(tag => tag.name !== newTagName);
