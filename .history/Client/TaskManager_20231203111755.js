        ...
        if (typeof localStorage !== 'undefined') {
            ...
            for (let i = 0; i < localStorage.length; i++) {
                ...
                if (key.startsWith('task-')) {
                    ...
                    // Convert description, list, and dueDate to strings if they are arrays
                    if (Array.isArray(taskData.description)) {
                        taskData.description = taskData.description.join(' ');
                    }
                    if (Array.isArray(taskData.list)) {
                        taskData.list = taskData.list.join(' ');
                    }
                    if (Array.isArray(taskData.dueDate)) {
                        taskData.dueDate = taskData.dueDate.join(' ');
                    }
