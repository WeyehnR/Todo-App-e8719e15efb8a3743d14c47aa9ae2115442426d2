...
export default class TaskUI {
    constructor(taskManager, domHelper, ui) {
        ...
        this.createTaskDetailsElements();
    }
    ...
    addArrowButtonEventListener(arrowButton, task, taskElement) {
      ...
      addSubtaskEventListener() {
        ...
        createTaskDetailsElements() {
            // Create a new HTML element for the date
            this.dateElement = document.createElement('p');
            this.dateElement.classList.add('date-text');
            this.taskElement.appendChild(this.dateElement);
    
            // Create a new HTML element for the time
            this.timeElement = document.createElement('p');
            this.timeElement.classList.add('time-text');
            this.taskElement.appendChild(this.timeElement);
    
            // Create a new HTML element for the list name
            this.listNameElement = document.createElement('p');
            this.listNameElement.classList.add('list-name');
            this.taskElement.appendChild(this.listNameElement);
    
            // Create a new HTML element for the subtask counter
            this.subtaskCounterElement = document.createElement('p');
            this.subtaskCounterElement.classList.add('subtask-counter');
            this.taskElement.appendChild(this.subtaskCounterElement);
        }
    
        updateTaskDetails(dateSelector, timeSelector, listDropdown) {
            // Add an event listener to the date selector
            dateSelector.addEventListener('change', (event) => {
                // Update the date text in the task list item
                this.dateElement.textContent = event.target.value;
            });
    
            // Add an event listener to the time selector
            timeSelector.addEventListener('change', (event) => {
                // Update the time text in the task list item
                this.timeElement.textContent = event.target.value;
            });
    
            // Add an event listener to the list dropdown menu
            listDropdown.addEventListener('change', (event) => {
                // Update the list name in the task list item
                this.listNameElement.textContent = event.target.value;
            });
        }
    
        updateSubtaskCounter() {
            // Update the subtask counter in the task list item
            this.subtaskCounterElement.textContent = `${this.task.subtasks.length} Subtask`;
        }
