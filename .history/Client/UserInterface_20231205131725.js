...
export default class UI {
    ...
    bindUIEventListeners() {
        ...
        const oldListener = todoList.getAttribute('data-listener');
        if (oldListener) {
            todoList.removeEventListener('click', this[oldListener]);
            delete this[oldListener];
        }

        // Add a new click event listener
        const newListener = `listener_${Date.now()}`;
        this[newListener] = (event) => this.expandRightMenu(event);
        todoList.addEventListener('click', this[newListener]);
        todoList.setAttribute('data-listener', newListener);
