        ...
        arrowButton.addEventListener('click', () => {
            ...
            addSubtaskButton.addEventListener('click', () => {
                ...
                if (subtaskName) {
                    ...
                    // Find the task with the active ID
                    let task = tasks.find(task => String(task.id) === String(activeTaskId));
                    ...
                    if (task) {
                        ...
                        // Parse the tasks array from local storage
                        let tasks = JSON.parse(localStorage.getItem('tasks'));
                        console.log('tasks: ' + JSON.stringify(tasks, null, 2));
                        
                        let taskIndex = tasks.findIndex(task => String(task.id) === String(activeTaskId));

                        // Check if the task was found
                        if (taskIndex !== -1) {
                            // Update the task with the new task data
                            tasks[taskIndex] = task;

                            // Stringify the tasks array and save it back to local storage
                            localStorage.setItem('tasks', JSON.stringify(tasks));

                            // Get the subtasks of the task
                            let subtasks = task.subtasks;

                            // For each subtask in the subtasks array
