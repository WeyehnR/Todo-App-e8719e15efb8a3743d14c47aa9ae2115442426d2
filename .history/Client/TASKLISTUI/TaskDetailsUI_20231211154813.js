...
export default class TaskDetailsUI {
    ...
    initEventListeners() {
        ...
        const activeTaskId = localStorage.getItem('activeTaskId');
        if (activeTaskId) {
            this.loadTaskDetailsFromServer(activeTaskId);
        }
    }
    closeRightMenu() {
    ...
    loadTaskDetailsFromServer(taskId) {
        // console.log(`Loading task details for task with id ${taskId} from the server`);
        this.taskSender.getTaskFromServer(taskId)
            .then(task => {
                if (task) {
                    ...
                    // Load subtasks here if needed
                } else {
                    console.error(`Task with id ${taskId} not found on the server`);
                }
            })
