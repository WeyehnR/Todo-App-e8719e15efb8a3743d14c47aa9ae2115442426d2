        ...
        if (!this.bindDueDateCalled) {
            this.dueDateSelect.addEventListener('change', (event) => {
                ...
                if (updatedTask) {
                    ...
                    if (activeTask) {
                        const dueDateContainer = activeTask.querySelector('.due-date-container');
    
                        // Create a new <div> element for the due date container if it doesn't exist
                        if (!dueDateContainer) {
                            const newDueDateContainer = document.createElement('div');
                            newDueDateContainer.className = 'due-date-container';
                            activeTask.appendChild(newDueDateContainer);
                        }
    
                        const existingDueDateElement = activeTask.querySelector('.due-date');
                        if (existingDueDateElement) {
                            existingDueDateElement.textContent = selectedDueDate;
                        } else {
                            const dueDateElement = document.createElement('p');
                            dueDateElement.className = 'due-date';
                            dueDateElement.textContent = selectedDueDate;
                            dueDateContainer.appendChild(dueDateElement);
                        }
                    }
                } else {
                    console.error(`Task with id ${activeTaskId} not found`);
