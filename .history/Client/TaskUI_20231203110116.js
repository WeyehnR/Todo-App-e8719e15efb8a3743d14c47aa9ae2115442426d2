    ...
    addTaskEventListener() {
        ...
        if (addTaskBtn) {
            addTaskBtn.addEventListener('click', () => {
                ...
                const selectedList = Array.from(listSelect.selectedOptions).map(option => option.value);

                const dueDateSelect = this.domHelper.getElementById('due-date-select');
                const selectedDueDate = Array.from(dueDateSelect.selectedOptions).map(option => new Date(option.value));
    ...
    addTaskClickEventListener() {
        ...
        tasks.forEach(task => {
            task.addEventListener('click', () => {
                ...
                let taskRenameInput = t.domHelper.querySelector('.task-rename');
