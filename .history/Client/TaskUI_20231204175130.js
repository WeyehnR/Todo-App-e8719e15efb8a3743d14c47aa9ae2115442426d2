        ...
        tasks.forEach(task => {
            ...
            if (arrowBtn) {
                arrowBtn.addEventListener('click', () => {
                    ...
                    const taskName = task.querySelector('label').innerText;
                    taskRenameInput.value = taskName;
                    const taskId = task.getAttribute('data-task');
                    localStorage.setItem('selectedTaskId', taskId);
                    const taskInManager = this.taskManager.tasks.find(t => t._id === taskId);
                    if (taskInManager) {
                        taskInManager._name = taskName;
                        this.taskManager.saveTaskToLocalStorage(taskInManager); // update the task in localStorage
                        console.log('localStorage:', localStorage.getItem('tasks')); // add this line
                    }
                });
            }
