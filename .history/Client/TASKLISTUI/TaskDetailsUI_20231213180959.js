...
export default class TaskDetailsUI {
    ...
    bindDueDateSelect() {
        if (!this.bindDueDateCalled) {
            this.dueDateSelect.addEventListener('change', (event) => {
                ...
                const activeTaskId = localStorage.getItem('activeTaskId');
                // Send the updated task to the server
                const updatedTask = this.taskManager.findTaskById(Number(activeTaskId)); // findTaskById(
                ...
                // Set the flag to true to indicate that bindDueDate has been called
    ...
    bindTagsSelect() {
        this.tagsSelect.addEventListener('change', (event) => {
            ...
            const activeTaskId = localStorage.getItem('activeTaskId');
            // Send the updated task to the server
            const updatedTask = this.taskManagerer.findTaskById(activeTaskId);
            if (updatedTask) {
                this.taskSender.updateTaskOnServer(updatedTask);
            } else {
                console.error(`Task with id ${activeTaskId} not found`);
