...
class TaskManager {
    ...
    renderTasks() {
        ...
        this.tasks.forEach(task => {
            ...
            taskElement.querySelector('label').innerText = task.taskRenameInput || task.name;
        ...
        todoList.querySelectorAll('.arrow-btn').forEach((button, index) => {
            button.addEventListener('click', () => {
                // Get the task name from the correct task object
                taskRenameInput.value = self.tasks[index].taskRenameInput || this.tasks[index].name;
            });
        });
