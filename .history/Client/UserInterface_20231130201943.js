...
export default class UI {
    ...
    deleteList(name) {
        ...
        const listItems = Array.from(document.querySelectorAll('.Lists-menu-selection .new-list'));
        const listItem = listItems.filter(item => item.textContent.trim() === name)[0];
        if (listItem) {
            listItem.remove();
        }
    ...
    updateDropdownMenu() {
        ...
        // Populate the dropdown menu with the updated list
        for (let i = 0; i < updatedLists.length; i++) {
            let option = new Option(updatedLists[i].name, i);
