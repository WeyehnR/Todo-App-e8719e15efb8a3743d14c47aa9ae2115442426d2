...
class TaskManager {
    ...
    handleDOMContentLoaded = () => {
        ...
        taskRenameInput.value = renameValue || ''; // load the value of the input field from local storage
        const task = this.tasks.find(task => task.renameValue === renameValue);
        if (task) {
            this.updateUIWithTaskDetails(task); // update the UI with the details of the task
        }
    }
