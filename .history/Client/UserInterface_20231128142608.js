        ...
        this.getElementById('new-list-name').addEventListener('keypress', (event) => {
            if (event.key === 'Enter') {
                ...
                if (newListName) {
                    ...
                    newListElement.innerHTML = `<span class="icon new-list-icon"></span>${newListName}<span class="item-count"></span>`;
                    const icon = newListElement.querySelector('.new-list-icon');
                    icon.style.backgroundColor = dynamicColor;
