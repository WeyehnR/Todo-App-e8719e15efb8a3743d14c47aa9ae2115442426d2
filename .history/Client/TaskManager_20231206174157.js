...
class TaskManager {
    ...
    addTask(name, description = '', selectedList = 'Select List', selectedDueDate = 'Select Due Date', selectedTags = 'Select Tags') {
        ...
        if (!taskExists) {
            ...
            // console.log("New SubTaskManager:", newSubtaskManager); // Add this line
            
            // Store the SubTaskManager in the subTaskManagers object
            this.subTaskManagers[task._id] = newSubtaskManager; // Add this line
            // console.log("Added SubTaskManager:", this.subTaskManagers); // Add this line
    ...
    loadTasksFromLocalStorage() {
        try {
            ...
            // Render tasks after loading them
            this.tasks.forEach(task => {
                // Create a new SubTaskManager for the task
                const newSubtaskManager = new SubTaskManager(task, this.domHelper);
                newSubtaskManager.init();

                // Store the SubTaskManager in the subTaskManagers object
                this.subTaskManagers[task._id] = newSubtaskManager;

                // Render the task
                const taskElement = this.taskUI.createTaskElement(task);
                this.domHelper.appendTaskElement(taskElement);
            });
