export default class UI {
    ...
    toggleNewListForm() {
        return new Promise((resolve, reject) => {
            ...
            if (!addListElement) {
                reject('Add list element not found');
            } else {
    ...
    addNewList() {
        return new Promise((resolve, reject) => {
            const newListForm = this.getElementById('new-list-form');
            const newListInput = this.getElementById('new-list-input');
            const newListSubmitBtn = this.getElementById('new-list-submit-btn');

            if (!newListForm || !newListInput || !newListSubmitBtn) {
                reject('New list form elements not found');
            } else {
                newListSubmitBtn.addEventListener('click', (event) => {
                    event.preventDefault();
                    const listName = newListInput.value;
                    if (listName) {
                        const list = new List(listName);
                        this.taskManager.addList(list)
                            .then(() => {
